  ...
  return (
    <Box sx={{
      ...
      <Box sx={{ position: "relative" }}>
        ...
        {/* Swipe background — left side */}
        <Box sx={{
          position:        "absolute",
          inset:           0,
          background:      `linear-gradient(to right, #e53935 ${progress * 100}%, transparent 100%)`,
          display:         "flex",
          alignItems:      "center",
          justifyContent:  "flex-start",
          pl:              2,
          opacity:         isActive && offset > 0 ? 1 : 0,
          pointerEvents:   "none"
        }}>
          <RemoveShoppingCartIcon sx={{
            color:      "white",
            fontSize:   26,
            transform:  `scale(${0.5 + progress * 0.5}) translateX(${(1 - progress) * -12}px)`,
            transition: dragging.current ? "none" : "transform 0.25s ease",
            opacity:    progress,
          }}/>
        </Box>

        {/* Swipe background — right side */}
        <Box sx={{
          position:        "absolute",
          inset:           0,
          background:      `linear-gradient(to left, #e53935 ${progress * 100}%, transparent 100%)`,
          display:         "flex",
          alignItems:      "center",
            ...
            {loading && !product ? (
              ...
              <>
                ...
                <Stack direction="row" alignItems="center" justifyContent="space-between" mt={0.75}>
                  <Box>
                    <Typography variant="body2" fontWeight={700} color="primary" lineHeight={1}>
                  ...
                  <ButtonGroup size="small" variant="outlined" sx={{
                    borderRadius: 0,
                    "& .MuiButtonGroup-grouped": { borderRadius: 0 },
                    "& .MuiButton-root.Mui-disabled": { borderColor: "primary.main", color: "primary.main" }
                  }}>
                    ...
                    <Button onClick={() => onSetQty(pid, qty - 1)} sx={{ px: 0.75, minWidth: 30 }}>
                      {qty === 1
                        ? <RemoveShoppingCartIcon sx={{ fontSize: 14 }}/>
    ...
    <Box sx={{ pb: 4 }}>
      ...
      {sortedList.map(({ pid, qty }) => (
        <SwipeableCartItem
          ...
          onNavigate={(pid) => navigate(`/view/${pid}`)}
          onSetQty={setQty}
        />
      ))}

      <Stack direction="row" justifyContent="space-between" alignItems="baseline" sx={{ px: 2, pt: 2, pb: 1 }}>
        <Typography color="text.secondary" variant="body2">{count} item{count !== 1 ? "s" : ""}</Typography>
        <Stack direction="row" alignItems="baseline" gap={0.5}>
          <Typography variant="caption" color="text.secondary">Total</Typography>
          <Typography variant="h6" fontWeight={800} color="primary">৳{total.toLocaleString()}</Typography>
        </Stack>
      </Stack>
